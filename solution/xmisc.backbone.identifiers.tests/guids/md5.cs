...
namespace xmisc.backbone.identity.tests.guids
{
    public class Md5GuidTests
    {
        ...
        {
            //act
            var first = Md5Guid.NewGuid(Fixture.Encoding.GetBytes(Fixture.NamespaceId), Fixture.Encoding.GetBytes(x));
            var second = Md5Guid.NewGuid(Fixture.Encoding.GetBytes(Fixture.NamespaceId), Encoding.Unicode.GetBytes(y));
        ...
        {
            //arrange
            var guid = Md5Guid.NewGuid(Fixture.NamespaceId, name, Fixture.Encoding);
